...
class PluginManager {
    ...
    static function allInstalled() {
        ...
        while ($ht = db_fetch_array($res)) {
            ...
            $info = static::getInfoForPath(
                OSTA_INCLUDE_DIR . $ht['install_path'], $ht['isphar']);
            ...
            if (!isset($class))
                $class = $path ?? null;
            elseif ($ht['isphar'])
                @include_once('phar://' . OSTA_INCLUDE_DIR . $ht['install_path']
                    . '/' . $path);
            else
                @include_once(OSTA_INCLUDE_DIR . $ht['install_path']
                    . '/' . $path);
    ...
    static function allInfos() {
        foreach (glob(OSTA_INCLUDE_DIR . 'plugins/*',
    ...
    static function getInfoForPath($path, $is_phar=false) {
        ...
        $install_path = str_replace(OSTA_INCLUDE_DIR, '', $path);
